...
function ChatRoom() {
  return (
    <div className="flex flex-col flex-grow w-full max-w-xl bg-white shadow-xl text-gray rounded-lg overflow-hidden">
      <Title text="SPOT AI Chat" />
      <MessageHistory />
      <MessageInput />
    </div>
  );
...
function OutboundMessage({ text, mins_ago }) {
  return (
    <div className="flex w-full mt-2 space-x-3 max-w-xs">
      ...
      <div>
        <div className="bg-green-500 p-3 rounded-r-lg rounded-bl-lg">
...
function InboundMessage({ text, mins_ago }) {
  return (
    <div className="flex w-full mt-2 space-x-3 max-w-xs ml-auto justify-end">
      <div>
        <div className="bg-purple-800 text-white p-3 rounded-l-lg rounded-br-lg">
  ...
  return (
    <div className="flex flex-col flex-grow h-0 p-4 overflow-auto">
      {messageHistory.map((message) => {
        return message.messageType == "outbound" ? (
          <InboundMessage
  ...
  return (
    <div className="bg-green-100 p-4">
      <form onSubmit={handleSubmit}>
        <input
          className="flex items-center h-10 w-full rounded px-3 text-sm text-gray"
          type="text"
          id="message"
          name="message"
          required
